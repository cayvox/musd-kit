...
import { drawForMargin, evaluateOpen, getAddresses, getBorrowingPower } from '../src'
import type { MathDeps } from '../src/math/deps'

/** A margin for the tests that need a second solve (MK-240): arbitrary inputs, not advice. */
const MARGIN = { horizonSeconds: 3600n, priceFallBps: 200n } as const
...
describe('MK-119, MK-118, a fee the chain does not charge linearly', () => {
  ...
  it('and so is a margin draw, at the stressed price', async () => {
    const { deps: d } = deps(price, doubled)
    const p = await drawForMargin(d, { collateral, ...MARGIN })
    const expected = largestOpen(collateral, p.margin.stressedPrice, doubled, 0n)
    expect(p.draw).toBe(expected < p.ceiling ? expected : p.ceiling)
  })
  ...
  it('each search stops when it has converged, so the reads are bounded by the range and not by the cap', async () => {
    ...
    await drawForMargin(d, { collateral, ...MARGIN })
...
describe('MK-118, no fee is asked for a figure that cannot exceed the reserve', () => {
  ...
  it('a collateral whose ICR cap is exactly the gas reserve reads no fee', async () => {
    ...
    const p = await drawForMargin(d, { collateral: 22n * 10n ** 14n, ...MARGIN })
    expect([p.ceiling, p.draw]).toEqual([0n, 0n])
    expect(feeReads, 'nothing to quote a fee for').toEqual([])
  })

  it('a ceiling under the floor asks nothing more for a margin draw', async () => {
    // 0.011 BTC caps the debt at 1,000 MUSD, under the 1,800 floor, so the ceiling is zero. On the search
    // path a margin draw could cost a second search; with a zero ceiling it must not run.
    const small = 11n * 10n ** 15n
    const { deps: d, feeReads } = deps(price, doubled)
    const p = await drawForMargin(d, { collateral: small, ...MARGIN })
    expect(p.ceiling).toBe(0n)
    // The same call with a stressed price whose cap cannot clear the reserve at all: the margin leg is
    // skipped there for that reason, so any difference in reads is the margin draw's search.
    const { deps: d2, feeReads: reads2 } = deps(price, doubled)
    await drawForMargin(d2, {
      collateral: small,
      priceFallBps: 9_999n,
      horizonSeconds: 0n,
    })
  ...
  it('a stressed cap of exactly the reserve reads no fee for a margin draw', async () => {
    ...
    const p = await drawForMargin(at.deps, {
      collateral,
      priceFallBps: 9_780n,
      horizonSeconds: 0n,
    })
    ...
    await drawForMargin(below.deps, {
      collateral,
      priceFallBps: 9_781n,
      horizonSeconds: 0n,
    })
