...
**It runs, and the figures below were measured on this tree** (P15 wave, 111 minutes, four slices).
...
one.
| Preview verdict against actual transaction outcome, swept | **Reproducible:** `MK_DIFF_CASES=1000 MK_DIFF_SEED=20260826 pnpm test:fork`, four slices of 250 via `MK_DIFF_FROM` and `MK_DIFF_TO`, a fresh anvil each at pinned block 15043414. Measured on THIS tree in the P15 wave, ten operations, **111 minutes across the four slices**: `0..250` 1593s ran=250 skipped=20; `250..500` 1697s ran=250 skipped=25; `500..750` 1719s ran=250 skipped=26; `750..1000` 1648s ran=250 skipped=26. **Totals: 1000 ran, 97 skipped, FALSE_VIABLE=0, FALSE_BLOCKED=10, NUMBERS=0, threw=0.** Bands boundary=580 extreme=214 middle=206, which is the 60/20/20 weighting. Recovery Mode reached 254 of 1000; per operation, executions in Recovery Mode were open=39, refinance=28, adjust=28, addCollateral=25, withdrawCollateral=24, repay=22, borrow=20, close=20, borrowingPower=15, **redeem=0 (26 skipped)**. Recovery Mode BORROWS by band: boundary=8, extreme=9, middle=3. Redemption bands ran/skipped: AT_NET_DEBT 13/12, WITHIN_HEADROOM 11/11, WHOLE_TROVE 11/9, AT_HEADROOM 9/10, IN_THE_GAP 8/5. It does NOT reach liquidation or claim, which have no preview to compare | **The four slices exit 1, and the reason is the harness rather than the SDK (MK-079).** All ten FALSE_BLOCKED are one defect: `adjustCase` passes its legs verbatim to the preview and filters them on `> 0n` before the write, so for an `adjust` case whose generated debt is under 4 wei the preview is asked about a zero debt increase (correctly `ZERO_DEBT_INCREASE`) while the chain is asked for a pure collateral top-up (correctly accepted). Ten tuples, one cause, no `packages/*/src` file implicated. **`FALSE_VIABLE=0` is the direction that would cost a user a failed transaction, and it is zero across all 1000.** **A band that never ran proves nothing**, so every count above is printed rather than summarised: `redeem` executed in Recovery Mode zero times out of 26 such cases, so this sweep says nothing about that combination. `threw=0` matters too, since throws are counted and never fatal, so a sweep can exit 0 with cases that never reached the chain |
