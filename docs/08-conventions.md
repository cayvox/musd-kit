...
| 14 | **When a wave changes what a finding SAYS, check that finding's summary table row and the register's opening section against it in the same wave** (MK-079's row described a mechanism MK-254 had emptied, MK-060's described presence after MK-244 made it value, and the opening section twice stated counts and an ID range the table contradicts) | The row, the opening section and the entry agree, or the wave says which one it deliberately left and why |
